from proto.Device_pb2 import DeviceInfo, DeviceType, PowerState, GetDevicesResponse, Empty
...
class ResponseParser:
    ...
    def parse_device_info(device_info: DeviceInfo):
        return f"Device {device_info.id.value}\n" + \
    ...
    def parse_get_devices_response(get_devices_response: GetDevicesResponse):
        return "\n\n".join([ResponseParser.parse_device_info(dev_info) for dev_info in get_devices_response.devices])

    def parse_lamp_info(lamp_info: LampInfo):
        return f"Lamp {lamp_info.deviceInfo.id.value}\n" + \
    ...
    def parse_fridge_info(fridge_info: FridgeInfo):
        return f"Fridge {fridge_info.deviceInfo.id.value}\n" + \
