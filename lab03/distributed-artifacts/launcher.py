...
def main():
    replica_count, segments_number, data_nodes_number, error = parse_arguments(arguments=sys.argv[1:])
    if error is None:
    ...
    replica_count, segments_number, data_nodes_number = inject_default_values(replica_count, segments_number, data_nodes_number)

    print(f"Starting Distributed-Artifact system with {data_nodes_number} data nodes...")
    ray.init()
    data_nodes = [DataNode.remote(id) for id in range(data_nodes_number)]
    name_node = NameNode.remote(data_nodes, replica_count = replica_count, artifact_segment_number = segments_number)
...
def parse_arguments(arguments: list[str]) -> tuple[int, int, int, str]:
    ...
    replica_count, segments_number, data_nodes_number = input_numbers + [None for _ in range(3 - len(arguments))]

    return replica_count, segments_number, data_nodes_number, None


def validate_arguments(replica_count: int, segments_number: int, data_nodes_number: int) -> str:
    '''Returns: (error: str)'''

    if replica_count is not None and segments_number is not None and data_nodes_number is not None and replica_count * segments_number > data_nodes_number:
        return f"`DATA_NODES_NUMBER` cannot be smaller, than expression `REPLICA_COUNT * SEGMENTS_NUMBER`"
    
    return None

def inject_default_values(replica_count: int, segments_number: int, data_nodes_number: int) -> tuple[int, int, int, str]:
    ...
    if data_nodes_number is None:
        data_nodes_number = max(DEFAULT_DATA_NODES_NUMBER, replica_count * segments_number)
