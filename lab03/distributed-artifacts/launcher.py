...
def parse_arguments(arguments) -> tuple[int, str]:
    if len(arguments) > 1:
        return None, f"Server launcher takes only 1 optional argument: number of data nodes, but {len(arguments)} were given: '{arguments}'"
    ...
    try:
        ...
        return None, f"Expected integer argument (number of data nodes), but '{node_num_arg}' was provided"
    
    if node_num <= 0:
        return None, f"Expected positive integer argument (number of data nodes), but '{node_num}' was provided"
