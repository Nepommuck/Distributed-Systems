...
class Artifact:
    ...
    def split_into_parts(self, segments_number: int) -> list["ArtifactSegment"]:
        ...
        segment_final_lengths = [segment_length + (1 if i < remaining_length else 0) for i in range(segments_number)]

        start_indexes = [0]
        for segment_length in segment_final_lengths[:-1]:
            start_indexes.append(start_indexes[-1] + segment_length)

        contents = [self.content[start : start + length] for start, length in zip(start_indexes, segment_final_lengths)]
        result = [ArtifactSegment(self.name, segment_index=i, content=content) for (i, content) in enumerate(contents)]
...
class ArtifactSegment:
    def __init__(self, artifact_name: str, segment_index: int, content: str) -> None:
        assert (
            artifact_name is not None and content is not None and segment_index is not None
