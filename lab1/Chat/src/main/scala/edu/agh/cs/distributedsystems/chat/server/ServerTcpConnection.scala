...
class ServerTcpConnection(val clientSocket: Socket, val server: Server) extends Runnable with Logging {
  ...
  def sendMessage(protocolMessage: ProtocolMessage): Unit = {
    protocolMessage.encode match {
      ...
      case None => logger.error(s"Failed to encode message: $protocolMessage")
    }
  }
  ...
  private def listenForMessages(): Unit = {
    ...
    if (receivedRawMessage != null) {
      ProtocolMessage.decodeRawMessage(rawMessage = receivedRawMessage) match {
        ...
        case _ =>
          logger.error(s"Received invalid message: `$receivedRawMessage`")
      }
      listenForMessages()
    }
  }
